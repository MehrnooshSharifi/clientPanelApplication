...
export default async function handler(req, res) {
  ...
  const { typeId, title, usersender, ticketdoc, ticketFiles } = req.body;

  if (!typeId || !title || !usersender || !ticketdoc || !ticketFiles) {
  ...
  try {
    const url = `${process.env.NEXT_PUBLIC_CLIENT_URL}/api/v1/Ticket/createNewClientTicket`;
