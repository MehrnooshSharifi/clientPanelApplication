...
export default async function handler(req, res) {
  ...
  const { amount, nationalCode, pathName } = req.body;

  if (!amount || !nationalCode || !pathName) {
  ...
  try {
    const url = `${process.env.NEXT_PUBLIC_CLIENT_URL}/IPG/v1/Payment/PaymentRequest`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ amount, nationalCode, pathName }),
    });
    if (!response.ok) {
